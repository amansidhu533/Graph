...
import { fetchDBData, fetchQueryResponse } from "../modules"
...
function ChatWindow() {
    ...
    useEffect(() => {
        localStorage.setItem("chatState", JSON.stringify({ fileData, queries, textArea1 }))
    }, [fileData, queries, textArea1])

    const handleDeleteChat = (fileNameToDelete) => {
        const updatedFiles = uploadedFiles.filter((file) => file.fileName !== fileNameToDelete)
        setUploadedFiles(updatedFiles)
    ...
    const handleDBConnectChartClick = (data, fileName) => {
      setDbChartData(data)  // Sets DBConnect data for the chart
      setJsonChartData(null) // Clears any JSON chart data
      setSelectedFileName(fileName) // Sets the selected file name
  
      // Find existing queries for this DB file, if any
      const selectedFile = uploadedFiles.find(
    ...
    const handleFileSubmit = (fileName, filePreview, selectedFileIndex = null) => {
    ...
    const handleDBClick = async () => {
        ...
        try {
            ...
            alert(`Failed to connect to database: ${error.response?.data?.message || error.message}`);
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleFileClick = (data, queries, fileName) => {
        ...
        const selectedFile = uploadedFiles.find((file) => file.fileName === fileName)
        const selectedQueries = selectedFile ? selectedFile.queries : []
        setFileData(data)
        setQueries(selectedQueries)
